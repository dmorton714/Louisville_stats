...
def check_data_updated():
    ...
    url = "https://data.louisvilleky.gov/datasets/8bd82421c9b94c37925fb37edaa1c5e8_0/explore" # noqa
    ...
    # Targeting the specific list item that contains the
    # "Data Updated" date and label
    data_updated_item = driver.find_element(By.CSS_SELECTOR, "li.metadata-item[data-test='modified']") # noqa
...
def process_salary_data(data, date_div):
    '''
    This function processes the salary data by
    ...
    Args:
        ...
        date_div (str): The date string (from the check_data_updated function)
        to determine the week and year
    ...
    # Check if there are any rows where the
    # CalYear is equal to the specified c_year
    if (data['CalYear'] == c_year).any():
        # Convert Annual_Rate to weekly rate by dividing by 52
        data.loc[data['CalYear'] == c_year, 'Annual_Rate'] = data.loc[data['CalYear'] == c_year, 'Annual_Rate'] / 52 # noqa

        # Scale the weekly rate for the specific week
        data.loc[data['CalYear'] == c_year, 'Annual_Rate'] = data.loc[data['CalYear'] == c_year, 'Annual_Rate'] * week # noqa

    return data


def plot_info(year, data) -> None:
    gauge = data.groupby(['CalYear'])[['YTD_Total',
                                       'Salary_Total']].sum().reset_index()
    ...
    fig = go.Figure(go.Indicator(
        ...
        title={'text': f"{year} Budgeted Salary Spend vs. Salary Spend Actual",
               'font': {'size': 20}},
    ))
...
def top_emp_dev(year, data):
    # we had to avoid division by zero and only calculate
    # Discrepancy_Percent for employees with Salary_Total >= 20k
    # Filter the data for the given year
    top_employee = data[data['CalYear'] == year].copy()

    # Calculate the discrepancy and discrepancy ratio
    top_employee['Deviation'] = top_employee['YTD_Total'] - top_employee['Salary_Total'] # noqa

    # Avoid division by zero and handle Salary_Total < 20k
    top_employee.loc[:, 'Discrepancy_Percent'] = top_employee.apply(
        lambda row: (row['Deviation'] / row['Salary_Total']) * 100 if row['Salary_Total'] >= 20000 and row['Salary_Total'] != 0 else None, axis=1 # noqa
    )

    # Round the Discrepancy_Percent to 2 decimal places
    top_employee['Discrepancy_Percent'] = top_employee['Discrepancy_Percent'].round(2) # noqa

    # Sort the DataFrame by 'Discrepancy_Percent' in descending order
    top_employee = top_employee.sort_values(by='Discrepancy_Percent', ascending=False) # noqa
...
def department_discrepancy(year, data):
    # Group by 'CalYear' and 'Department',
    # summing 'YTD_Total' and 'Salary_Total'
    department = data.groupby(['CalYear', 'Department'])[['YTD_Total', 'Salary_Total']].sum().reset_index() # noqa

    # Filter the data by the given year
    department = department[department['CalYear'] == year]

    # Calculate the percentage difference:
    # ((YTD_Total - Salary_Total) / Salary_Total) * 100
    department['Discrepancy_Percent'] = ((department['YTD_Total'] - department['Salary_Total']) / department['Salary_Total']) * 100 # noqa

    # Round the Discrepancy_Percent to 2 decimal places
    department['Discrepancy_Percent'] = department['Discrepancy_Percent'].round(2) # noqa

    # Sort the DataFrame by 'Discrepancy_Percent' in ascending order
    department = department.sort_values(by='Discrepancy_Percent', ascending=False) # noqa

    # Format 'YTD_Total' and 'Salary_Total' with thousands separators
    department['YTD_Total'] = department['YTD_Total'].apply(lambda x: f"{x:,.2f}") # noqa
    department['Salary_Total'] = department['Salary_Total'].apply(lambda x: f"{x:,.2f}") # noqa
...
def main():
    # Call the function to check data update information and capture the date
